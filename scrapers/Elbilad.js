...
async function run(choosenDate, choosenCategory, saveOption) {
  ...
  try {
    ...
    while (keepGoing) {
      ...
      ); // to know the last article's number and use it as an id
      ...
      if (choosenDate <= stopDate) {
        ...
        await page.waitForFunction(
          // so it doesn't scrape until  it loads more news when the button is clicked
          (prevCount) => {
            return (
        ...
        let mainArticl = await page.$eval(
          "#content > header > ul > li > h1 > a",
          (mainArticl) => {
        ...
        for (let articl of articles) {
          if (articl.link) {
            ...
            if (!articl.date) {
              ...
              articl.date = main$(
                "#content > header > ul.list-share > li.title > a > span:nth-child(2)"
              )
                .text()
                .trim()
                .slice(0, 10);
            ...
            if (choosenDate <= dateObj) {
              ...
              const author =
                $(".title").find(".strong")?.text() || "Unknown author";
              scrapedData.push({
                title: articl.title,
                link: articl.link,
                date: articl.date,
