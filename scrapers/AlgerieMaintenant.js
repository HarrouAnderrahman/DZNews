...
async function run(choosenDate, choosenCategory, saveOption) {
  ...
  try {
    ...
    await page.goto(
      `https://algeriemaintenant.dz/category/${choosenCategory}/`
    );

    console.log("Accessing Algeriemaintenant ...");

    await page.waitForSelector(
      "body > div.arcv > div > div > div.arcv__main > div > div.arcv__ccar"
    ); // waiting for the news to load so it doesn't mess up
    ...
    while (keepGoing) {
      ...
      if (choosenDate <= stopDate) {
        ...
        let scraped = await page.$$eval(
          "body > div.arcv > div > div > div.arcv__main > div > div.arcv__ccar > article.art1",
          (news) =>
            news.map((articl) => {
              const title =
                articl.querySelector("h2")?.textContent.trim() || "";
              const link = articl.querySelector("h2 > a")?.href || "";
              const date =
                articl.querySelector("div > div.datl.d-f > time")?.dateTime ||
                "";
        ...
        for (let articl of scraped) {
          if (articl.link) {
            ...
            if (choosenDate < dateObj) {
              ...
              const contentData = $(".article__txt")
                .text()
                .replace("\n\t\t\t\t\t\t\t\t", "");
              let author = $(".article__author-name")
                .text()
                .replace("بواسطة", "");
